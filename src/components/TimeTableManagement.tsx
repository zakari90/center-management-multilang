...
interface ScheduleSlot {
  ...
  syncError?: string; // Added syncError
}
...
export default function TimetableManagement({
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const scheduleSlots: ScheduleSlot[] = filteredSchedules.map((s) => ({
        ...
        syncError: s.syncError, // Map syncError
      }));
  ...
  const getSlotsByDayAndTime = (day: string, time: string) => {
    ...
    // Deduplicate: If we have a 'synced' slot (status '1') and a 'waiting' slot (status 'w' or '0')
    // for the same teacher/room, we should hide the waiting one properly, or just show unique ones.
    // The issue usually manifests as a locally created schedule (w) that failed to sync (409)
    // but the server already had it (so a '1' copy also came down).

    // Group by teacherId to find duplicates for the same teacher slot
    const uniqueSlots: ScheduleSlot[] = [];
    const processedTeachers = new Set<string>();

    // Sort: Synced ('1') first, then others. This ensures we pick the stable one.
    ...
    sortedSlots.forEach((slot) => {
      // If we already processed this teacher for this time slot, skip (it's a duplicate)
      if (processedTeachers.has(slot.teacherId)) return;

      uniqueSlots.push(slot);
      processedTeachers.add(slot.teacherId);
    });
                                ...
                                >
                                  <div className="flex justify-between items-start">
                                    ...
                                    <Button
                                      ...
                                      onClick={() =>
                                        slot.id && handleDeleteSchedule(slot.id)
                                      }
                                    >
