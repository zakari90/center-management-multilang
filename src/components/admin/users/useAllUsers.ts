  ...
  const fetchAllData = useCallback(async () => {
    ...
    try {
      ...
      const teachersData: TeacherData[] = activeTeachers.map((teacher) => {
        const manager =
          activeUsers.find((u) => u.id === teacher.managerId) ||
          (user?.id === teacher.managerId ? user : null);
      ...
      const studentsData: StudentData[] = activeStudents.map((student) => {
        const manager =
          activeUsers.find((u) => u.id === student.managerId) ||
          (user?.id === student.managerId ? user : null);
