...
export default function TimetableManagement({
  ...
  const queryData = useLiveQuery(async () => {
    try {
      ...
      // 2. Filter teachers (simplified for local admin mode)
      const adminTeachers = allTeachers.map((t: any) => ({
        id: t.id,
        name: t.name,
      ...
      const adminSubjects = allSubjects
        .filter((s: any) => s.centerId && targetCenterIds.includes(s.centerId))
      ...
      const finalRooms = allCenters
        .filter((c: any) => targetCenterIds.includes(c.id))
        .flatMap((c: any) => c.classrooms || [])
      ...
      return {
        ...
        rooms: finalRooms,
        teacherSubjects: allTeacherSubjects.map((ts: any) => ({
  ...
  if (authLoading || queryData === undefined) {
    return (
                    ...
                    {daysOfWeek.map((day: any) => {
                      ...
                      const currentTeacher =
                        viewMode === "teacher" && selectedFilter
                          ? teachers.find((t: any) => t.id === selectedFilter)
                          : null;
            ...
            <div className="space-y-4">
              <div className="space-y-2">
                ...
                <div className="flex items-center gap-2 p-3 bg-muted rounded-md">
                  ...
                  <span>
                    {teachers.find(
                      (t: any) => t.id === selectedScheduleDetails.teacherId,
                    )?.name || "N/A"}
                  </span>
                </div>
              </div>
              ...
              <div className="space-y-2">
                ...
                <div className="p-3 bg-muted rounded-md">
                  <div className="flex items-center justify-between">
                    <span>
                      {subjects.find(
                        (s: any) => s.id === selectedScheduleDetails.subjectId,
                      )?.name || "N/A"}
                    </span>
                    {subjects.find(
                      (s: any) => s.id === selectedScheduleDetails.subjectId,
                      ...
                      <Badge variant="secondary">
                        {
                          subjects.find(
                            (s: any) =>
                              s.id === selectedScheduleDetails.subjectId,
                          )?.grade
                        }
                      </Badge>
                    )}
