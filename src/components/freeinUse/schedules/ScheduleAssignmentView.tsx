...
export default function ScheduleAssignmentView({
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      // Show all teachers in local-first admin-only mode
      setTeachers(
        allTeachers.map((t) => ({
          id: t.id,
          name: t.name,
      ...
      // Show subjects for current center or all if no centerId
      const relevantSubjects = allSubjects
        .filter((s) => !centerId || s.centerId === centerId)
        .map((s) => ({ id: s.id, name: s.name, grade: s.grade }));
      setSubjects(relevantSubjects);

      // Show schedules for current center
      const filteredSchedules = allSchedules.filter(
        (s) => !centerId || s.centerId === centerId,
      );
      setSchedule(
      ...
      if (centerId) {
        ...
        const allAdminClassrooms = allCenters
          .flatMap((c) => c.classrooms || [])
          .filter((room, index, self) => self.indexOf(room) === index);

        if (allAdminClassrooms.length > 0) setRooms(allAdminClassrooms);
        else setRooms(availableClassrooms);
      }
    } catch (err) {
  ...
  }, [user, user?.id, authLoading, centerId, availableClassrooms]);
  ...
  const handleSlotClick = (room: string, startTime: string) => {
    ...
    const endTime = TIME_SLOTS[endTimeIndex] || "19:00";
    ...
    try {
      ...
      const teacherConflict = activeSchedules.find(
        (s) =>
          ...
          s.id !== (conflictingScheduleIds[0] || "new"),
      );
      ...
      if ((teacherConflict || roomConflict) && !force) {
        const conflicts = (
          [teacherConflict, roomConflict].filter(Boolean) as any[]
        ).map((c) => c.id);
        setConflictingScheduleIds(conflicts);
        setError(
          teacherConflict
