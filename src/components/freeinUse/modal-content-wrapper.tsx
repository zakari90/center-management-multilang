...
export function ModalContentWrapper({
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        // Only block dynamic detail pages (contains an id segment after role)
        const parts = pathname.split("/").filter(Boolean);
        const roleIndex = parts.findIndex((p) => p === "admin");
        const looksDynamic = roleIndex >= 0 && parts.length > roleIndex + 2;
        if (!looksDynamic) {
          if (!cancelled) setIsOfflineBlocked(false);
          return;
        }
  ...
  const handleClose = () => {
    ...
    // Path format: /[locale]/admin/[resource]/[id]/...
    // We want to go back to: /[locale]/admin/[resource]
    let listPath = "";

    // Check for role routes
    const roleSegment = pathParts.find((p) => p === "admin");
    if (roleSegment) {
      const roleIndex = pathParts.indexOf(roleSegment);
      if (roleIndex >= 0 && pathParts.length > roleIndex + 1) {
        const resource = pathParts[roleIndex + 1];
        listPath = `/${locale}/${roleSegment}/${resource}`;
      }
    }
