...
import { AlertCircle, Trash2, Clock, User, BookOpen, Calendar, AlertTriangle } from "lucide-react"
...
import { scheduleActions, teacherActions, subjectActions } from "@/lib/dexie/dexieActions"
...
export function PendingSchedulesDialog({ onRefresh }: PendingSchedulesDialogProps) {
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="max-w-3xl max-h-[80vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            These schedules are waiting to sync or have conflicts. Review and resolve them to ensure proper synchronization.
          </DialogDescription>
        </DialogHeader>
        ...
        {loading ? (
          ...
          <div className="space-y-3">
            {pendingSchedules.map((schedule) => (
              <Card 
                key={schedule.id} 
                className={schedule.status === "0" ? "border-red-200 bg-red-50/50" : "border-yellow-200 bg-yellow-50/50"}
              >
                <CardHeader className="pb-3">
                  <div className="flex items-start justify-between">
                    ...
                    <Badge variant={schedule.status === "0" ? "destructive" : "secondary"}>
                      {schedule.status === "0" ? "Pending Deletion" : "Waiting to Sync"}
                    </Badge>
                  </div>
                </CardHeader>
                <CardContent className="space-y-3">
                  <div className="grid grid-cols-2 gap-3 text-sm">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="font-medium">{DAYS_MAP[schedule.day] || schedule.day}</span>
                    </div>
                    <div className="flex items-center gap-2">
