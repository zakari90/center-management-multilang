...
import { WeeklyScheduleSlot, Teacher, Schedule, TeacherWithSchedule } from './types'
import { parseWeeklySchedule, calculateHoursDifference, isWithinAvailability } from './utils'
      ...
      if (isAdmin) {
        if (centerId) {
          ...
          activeSchedules = activeSchedules.filter(s => 
            managerIds.includes(s.managerId) || s.centerId === centerId
          )
        } else {
        ...
        activeSchedules = activeSchedules.filter(s => s.managerId === user.id)
      }
