...
// ==================== MAIN COMPONENT ====================
export default function EditTeacherDialog({ teacherId, onTeacherUpdated, trigger }: EditTeacherDialogProps) {
  const t = useTranslations("EditTeacherPage")
  const { user } = useAuth()
          ...
          const teacherSubjectsData = allTeacherSubjects
            ...
            .map(ts => {
              ...
              return {
                ...
                compensationType: ts.percentage ? 'percentage' : 'hourly' as const
              }
            })
  ...
  const updateSubject = (index: number, field: keyof TeacherSubjectForm, value: any) => {
    setTeacherSubjects((prev) => {
      const updated = [...prev]
      updated[index] = { ...updated[index], [field]: value }
      return updated
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      for (const vs of validSubjects) {
        const existing = currentTeacherSubjects.find(cts => cts.subjectId === vs.subjectId)
          ...
          <>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="space-y-3">
                <h3 className="text-sm font-semibold text-muted-foreground">{t("basicInformation")}</h3>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  ...
                  <div className="space-y-1.5">
                    <Label htmlFor="edit-email" className="text-sm">{t("email")}</Label>
                    <Input
                      type="email"
                      id="edit-email"
                      name="email"
                  ...
                  <div className="space-y-1.5">
                    <Label htmlFor="edit-phone" className="text-sm">{t("phoneNumber")}</Label>
                    <Input
                      id="edit-phone"
                      name="phone"
                  ...
                  <div className="space-y-1.5">
                    <Label htmlFor="edit-address" className="text-sm">{t("address")}</Label>
                    <Input
                      id="edit-address"
                      name="address"
