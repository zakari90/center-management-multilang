    ...
    return (
      <div className="max-w-4xl mx-auto p-6">
        <div className="bg-red-50 border border-red-200 rounded-md p-4">
          <p className="text-red-800">{error || 'Receipt not found'}</p>
        </div>
      </div>
    )
  }
        ...
        <div className="bg-white rounded-lg shadow-lg border-2 border-gray-200 p-8">
          ...
          <div className="border-b-2 border-gray-300 pb-6 mb-6 flex justify-between items-start">
            <div>
              ...
              <p className="text-gray-600">{center?.name || 'Your Education Center'}</p>
              <p className="text-sm text-gray-500">{center?.address || '123 Education Street'}</p>
              {center?.phone && <p className="text-sm text-gray-500">Phone: {center.phone}</p>}
            </div>
            <div className="text-right">
              <div
                className={`inline-block px-4 py-2 rounded-lg text-sm font-semibold mb-2 ${
                  receipt.type === 'STUDENT_PAYMENT' ? 'bg-green-100 text-green-800' : 'bg-orange-100 text-orange-800'
                }`}
              >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
            {/* Payer */}
            <div>
              <h3 className="text-sm font-semibold text-gray-600 mb-3">PAYMENT FROM:</h3>
              <div className="bg-gray-50 p-4 rounded-lg space-y-2">
                <p className="font-semibold text-gray-900 text-lg">{payer?.name}</p>
                {receipt.student?.grade && <p className="text-sm text-gray-600">Grade: {receipt.student.grade}</p>}
                {payer?.email && <p className="text-sm text-gray-600">{payer.email}</p>}
                {payer?.phone && <p className="text-sm text-gray-600">Phone: {payer.phone}</p>}
              </div>
            </div>
            ...
            <div>
              ...
              <div className="bg-gray-50 p-4 rounded-lg space-y-2">
                ...
                <div className="flex justify-between">
                  ...
                  <span className="text-sm font-medium text-gray-900">{receipt.paymentMethod || 'N/A'}</span>
                </div>
                <div className="flex justify-between">
          ...
          <div className="border-t-2 border-gray-300 pt-6">
            <div className="flex justify-between items-center mb-4">
              <span className="text-xl font-semibold text-gray-700">AMOUNT PAID:</span>
              <span className={`text-4xl font-bold ${receipt.type === 'STUDENT_PAYMENT' ? 'text-green-600' : 'text-orange-600'}`}>
